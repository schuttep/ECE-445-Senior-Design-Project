...
bool boardHasExactlyOneKingEach(char board[8][8]) {
  ...
  for (int r = 0; r < 8; r++) {
    for (int c = 0; c < 8; c++) {
      if (board[r][c] == 'K') whiteKings++;
      if (board[r][c] == 'k') blackKings++;
    }
  }
...
int castleFlagIndex(bool white, bool kingSide) {
  if (white && kingSide) return 0;
  if (white && !kingSide) return 1;
  if (!white && kingSide) return 2;
  return 3;
}
...
bool canCastle(char board[8][8], bool white, bool kingSide, const bool castling[4]) {
  ...
  if (!castling[flagIndex]) return false;
...
bool basicMoveIsLegal(char board[8][8], int r1, int c1, int r2, int c2) {
  ...
  if (sameColor(piece, board[r2][c2])) return false;
...
void updateCastlingFlags(char before[8][8],
  ...
  if (movedPiece == 'R' && r1 == 7 && c1 == 7) newFlags[0] = false;
  if (movedPiece == 'R' && r1 == 7 && c1 == 0) newFlags[1] = false;
  if (movedPiece == 'r' && r1 == 0 && c1 == 7) newFlags[2] = false;
  if (movedPiece == 'r' && r1 == 0 && c1 == 0) newFlags[3] = false;

  // If a rook is captured on its original square, that castling right is lost
  if (capturedPiece == 'R' && r2 == 7 && c2 == 7) newFlags[0] = false;
  if (capturedPiece == 'R' && r2 == 7 && c2 == 0) newFlags[1] = false;
  if (capturedPiece == 'r' && r2 == 0 && c2 == 7) newFlags[2] = false;
  if (capturedPiece == 'r' && r2 == 0 && c2 == 0) newFlags[3] = false;
}
...
bool hasAnyLegalMove(char board[8][8], bool whiteToMove, const bool castling[4]) {
  ...
  for (int r1 = 0; r1 < 8; r1++) {
    for (int c1 = 0; c1 < 8; c1++) {
      char piece = board[r1][c1];
      if (!isPiece(piece)) continue;
      if (isWhitePiece(piece) != whiteToMove) continue;
      ...
      for (int r2 = 0; r2 < 8; r2++) {
        for (int c2 = 0; c2 < 8; c2++) {
          if (!basicMoveIsLegal(board, r1, c1, r2, c2)) continue;
...
String validateMoveAndReturnFEN(const String& beforeFEN,
  ...
  for (int r = 0; r < 8; r++) {
    for (int c = 0; c < 8; c++) {
      if (before[r][c] != after[r][c]) diffCount++;
    }
  }
  ...
  if (diffCount == 4) {
    ...
    if (whiteToMove && canCastle(before, true, true, castling)) {
      ...
      if (sameBoard(expected, after)) {
        for (int i = 0; i < 4; i++) newCastling[i] = castling[i];
        newCastling[0] = false;
        newCastling[1] = false;
  ...
  if (!valid && diffCount == 2) {
    ...
    for (int r = 0; r < 8; r++) {
      for (int c = 0; c < 8; c++) {
        if (before[r][c] != after[r][c]) {
          // Source square
          if (before[r][c] != '.' && after[r][c] == '.') {
            if (fromRow != -1) return "Invalid Move";
            fromRow = r;
            fromCol = c;
          }
          // Destination square
          else if (after[r][c] != '.') {
            if (toRow != -1) return "Invalid Move";
            toRow = r;
            toCol = c;
          }
        }
    ...
    if (!isPiece(movedPiece)) return "Invalid Move";
    if (isWhitePiece(movedPiece) != whiteToMove) return "Invalid Move";
    if (sameColor(movedPiece, capturedPiece)) return "Invalid Move";
    ...
    if (type == 'p' && (toRow == 0 || toRow == 7)) {
      if (!isValidPromotionPiece(promotionPiece)) return "Invalid Move";
